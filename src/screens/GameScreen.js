...
export default function GameScreen({ navigation, route }) {
  ...
  async function startGame() {
    ...
    if (mode === "story") {
      // Shuffle story order
      const indices = STORY_LINES.map((_, i) => i).sort(
        () => Math.random() - 0.5,
      );
  ...
  function handleStoryTap(emoId) {
    ...
    const storyIndex =
      storyOrderRef.current[storyIdx % storyOrderRef.current.length];
    ...
    if (emoId === story.answer) {
      ...
      if (newAnswered % 4 === 0) {
        ...
        showComboPopup(
          `${newWorld.emoji} Level ${nextLevel}!`,
      ...
      <SafeAreaView style={s.safe} edges={["left", "right", "bottom"]}>
        ...
        >
          ...
          {mode === "story" &&
            (() => {
              const si =
                storyOrderRef.current[
                  storyIdx % Math.max(storyOrderRef.current.length, 1)
                ];
                ...
                <View style={[s.storyCard, { backgroundColor: bgColor }]}>
                  <View style={s.storyCardHeader}>
                    ...
                    <View style={s.storyCardMeta}>
                      ...
                      <Text style={s.storyScore}>
                        Story {storiesAnswered + 1}
                      </Text>
                    </View>
                  </View>
                  ...
                  {storyResult === "correct" && (
                    <View style={s.storyResultBadge}>
                      <Text style={s.storyResultTxt}>
                        ✓ Correct! +{20 + streakRef.current * 5} pts
                      </Text>
                    </View>
                  )}
                  {storyResult === "wrong" && (
